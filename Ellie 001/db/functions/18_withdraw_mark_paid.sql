-- Admin: mark withdrawal paid; reduce locked only, ledger entry
CREATE OR REPLACE FUNCTION public.admin_withdraw_mark_paid(p_request_id UUID, p_paid_ref TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET locked_cents = locked_cents - v_row.amount_cents
  WHERE user_id = v_row.user_id;

  UPDATE public.withdrawal_requests
  SET status = 'paid', paid_ref = p_paid_ref, reviewed_at = now(), paid_at = now()
  WHERE id = p_request_id;
  ...
  INSERT INTO public.ledger (
    ...
    v_row.user_id, 'withdraw_paid', -v_row.amount_cents,
    v_avail, v_avail,
